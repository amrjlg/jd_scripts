...
更新地址：https://gitee.com/washala/jd_scripts/raw/master/jd_syj.js
已支持IOS双京东账号, Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
============Quantumultx===============
[task_local]
#赚京豆
10 0,7,23 * * * https://gitee.com/washala/jd_scripts/raw/master/jd_syj.js, tag=赚京豆, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_syj.png, enabled=true

================Loon==============
[Script]
cron "10 0,7,23 * * *" script-path=https://gitee.com/washala/jd_scripts/raw/master/jd_syj.js, tag=赚京豆

===============Surge=================
赚京豆 = type=cron,cronexp="10 0,7,23 * * *",wake-system=1,timeout=3600,script-path=https://gitee.com/washala/jd_scripts/raw/master/jd_syj.js

============小火箭=========
赚京豆 = type=cron,script-path=https://gitee.com/washala/jd_scripts/raw/master/jd_syj.js, cronexpr="10 0,7,23 * * *", timeout=3600, enable=true
 */
const $ = new Env('赚京豆');
...
!(async () => {
  ...
  console.log(`\n\n内部互助 【赚京豆(微信小程序)-瓜分京豆】活动(优先内部账号互助(需内部cookie数量大于${$.assistNum || 4}个)，如有剩余助力次数则给作者washala和随机团助力)\n`)
  for (let i = 0; i < cookiesArr.length; i++) {
    $.canHelp = true
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      ...
      if ($.canHelp) {
        ...
        if ($.authorTuanList.length) console.log(`开始账号内部互助 赚京豆-瓜分京豆 活动，如有剩余则给作者washala和随机团助力`)
        for (let j = 0; j < $.authorTuanList.length; ++j) {
          console.log(`账号 ${$.UserName} 开始给作者washala和随机团 ${$.authorTuanList[j]['assistedPinEncrypted']}助力`)
